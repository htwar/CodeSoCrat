...
\begin{itemize}
    ...
    \item Test secure password storage and migration: Ensure new local registrations store salted PBKDF2 hashes and legacy SHA-256 hashes are upgraded after successful login.
...
\begin{itemize}
    ...
    \item Test password hashing upgrade: Ensure new local registrations store salted PBKDF2 hashes and older SHA-256 hashes are rehashed after successful login.
\end{itemize}
...
\hline
MB-CS-07 & Use the skip link on the auth page or main workspace, then trigger success, timer, and error messages. & Skip links move focus to the intended panel and status or error messages are surfaced in accessibility-friendly regions. &  \\
\hline
\end{tabular}
